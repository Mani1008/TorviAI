...
async fn run_watcher_loop(running: Arc<AtomicBool>, app: AppHandle) {
    ...
    loop {
        ...
        let text = filter.redact_sensitive(&ctx.text_content);
        ...
        let content_type =
            classify_context(&ctx.app_name, &ctx.window_title, ctx.url.as_deref());
...
/// Classify the content type based on app name and optional URL.
fn classify_context(app_name: &str, _title: &str, url: Option<&str>) -> &'static str {
    if let Some(u) = url {
    ...
    match app.as_str() {
        a if a.contains("slack") || a.contains("discord") || a.contains("teams") => "chat",
